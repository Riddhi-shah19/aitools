...
const AI_API=({data}) => {
    ...
    const apiComponents=data.map((apis)=>(
        ...
        <img className='h-[7rem] w-[10rem] hover:scale-105 cursor-pointer drop-shadow-2xl' src={apis.image}/>
        <div className='text-white p-[10px] w-[12rem] '>
        <a className='hover:scale-105 hover:underline cursor-pointer text-center mt-2 text-xl' href={apis.link} target='_blank'>{apis.title}</a>
